module StandingsGenerators
  class GamesStandingsService < StandingsGeneratorService
    ...
    def build_data_array
      {}.tap do |games_won|
        game_winners.each do |game_winner|
          games_won[game_winner.player_id] ||= EMPTY_DATA_ENTRY.dup

          games_won[game_winner.player_id][:name]     = select_player(game_winner.player_id).player_name
          games_won[game_winner.player_id][:num_won] += 1
        end
